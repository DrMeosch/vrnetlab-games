...
add address=192.168.200.253/32 interface=bridge-lo0

/routing bgp connection
add connect=yes disabled=no listen=yes local.address=192.168.200.254 .role=ebgp multihop=yes name=toR1 output.network=r2-bgp-networks remote.address=\
    192.168.100.254/32 routing-table=rtable-special templates=default
add connect=yes disabled=no listen=yes local.address=192.168.97.254 .role=ebgp multihop=yes name=toK8S output.default-originate=always .network=\
    r2-bgp-networks remote.address=192.168.97.250/32 routing-table=rtable-special templates=default
add connect=yes disabled=no input.filter=default_fix_dist listen=no local.address=192.168.200.253 .role=ebgp multihop=yes name=toR1-main remote.address=\
    192.168.100.254/32 router-id=192.168.200.253 templates=default

/routing filter rule
add chain=default_fix_dist rule="if (dst == 0.0.0.0/0) {set distance 1; set scope 30; set scope-target 30; accept} else {accept}"

/routing rule
add action=lookup-only-in-table src-address=192.168.97.253/32 table=rtable-special
add action=lookup-only-in-table disabled=yes src-address=192.168.97.250/32 table=rtable-special
