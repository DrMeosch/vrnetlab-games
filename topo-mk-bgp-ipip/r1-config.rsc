...
add allow-fast-path=no \
    ...
    local-address=10.241.10.254 name=ipip-tunnel1 remote-address=10.241.10.253
/ip route
add dst-address=192.168.200.0/24 gateway=ipip-tunnel1

/interface bridge
add name=bridge-lo0

/ip address
add address=192.168.100.254/24 interface=bridge-lo0 network=192.168.100.0

/routing bgp connection
add disabled=no listen=yes local.role=ebgp name=toR2 output.default-originate=always \
    .network=r1-bgp-networks remote.address=192.168.200.254 templates=default \
    local.address=192.168.100.254 multihop=yes
