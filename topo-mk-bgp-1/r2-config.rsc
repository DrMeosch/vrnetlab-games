...
add disabled=no listen=yes local.role=ebgp name=toR1 output.network=r2-bgp-networks \
    remote.address=10.241.10.254 routing-table=rtable-special templates=default
add disabled=yes listen=yes local.role=ebgp-rs-client name=toR1-only remote.address=\
    10.241.10.254 routing-table=main templates=default
